...
*fetches* on load. Permitted, because the page never fetches them: plain `<a href>`
navigation links (contacted only when the user clicks), the `<link rel="canonical">`
URL declaration, crawler-read metadata (Open Graph / Twitter `<meta>` tags), and
JSON-LD structured-data blocks (data, never fetched or executed — CSP-inert).
This admits the footer's Ko-fi tip link, the k4-713.com breadcrumb, the GitHub
source link, and the discoverability layer. Scripts, styles, fonts, images, and
anything else auto-loaded stay same-origin only.)
